...
- (void)loadData {
    ...
    for (NSDictionary *dict in arrData) {
        ...
        [dic1 setObject:[dict objectForKey:@"name"] forKey:@"name"];
        [dic1 setObject:[dict objectForKey:@"id"] forKey:@"code"];
        NSMutableArray *cityArr = [dict objectForKey:@"child"];
        NSMutableArray *aaaa = [NSMutableArray array];
        for (NSDictionary *dict in cityArr) {
            NSMutableDictionary *dic2 = [NSMutableDictionary dictionary];
            [dic2 setObject:[dict objectForKey:@"name"] forKey:@"name"];
            [dic2 setObject:[dict objectForKey:@"id"] forKey:@"code"];
            NSMutableArray *cityArr = [dict objectForKey:@"child"];
            NSMutableArray *bbbb = [NSMutableArray array];
            for (NSDictionary *dict in cityArr) {
                NSMutableDictionary *dic3 = [NSMutableDictionary dictionary];
                [dic3 setObject:[dict objectForKey:@"name"] forKey:@"name"];
                [dic3 setObject:[dict objectForKey:@"id"] forKey:@"code"];
        ...
        CGXProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
            if (rowOfCity < provinceModel.city.count) {
                ...
                if (rowOfTown < cityModel.town.count) {
                    ...
                    arr = @[provinceModel.name, cityModel.name, townModel.name,provinceModel.code,cityModel.code,townModel.code];
                }
            }
    }
    return arr;
