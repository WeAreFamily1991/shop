...
- (MLSearchResultsTableViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.view.frame = CGRectMake(0, DRTopHeight, self.view.mj_w, self.view.mj_h);
...
- (void)viewDidLoad {
    ...
    //  第4部  调用创建
    YJSegmentedControl * segment = [YJSegmentedControl segmentedControlFrame:CGRectMake(0, DRTopHeight, self.view.bounds.size.width, 44) titleDataSource:btnDataSource backgroundColor:[UIColor whiteColor] titleColor:[UIColor blackColor] titleFont:[UIFont fontWithName:@".Helvetica Neue Interface" size:16.0f] selectColor:[UIColor redColor] buttonDownColor:[UIColor redColor] Delegate:self];
    // 第5步 添加到试图上
    [self.view addSubview:segment];
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, DRTopHeight+44, SCREEN_WIDTH, SCREEN_HEIGHT-DRTopHeight-44) style:UITableViewStylePlain];
    ...
    // 创建搜索框
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth-64-20-40, 40)];
    UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(-10, 0, titleView.frame.size.width, 40)
                              ];
