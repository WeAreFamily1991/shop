...
- (void)photoPickerDidMaximum:(AJPhotoPickerViewController *)picker {
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"最多可选择%zd张照片", selectMaxNum]  delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
    [alertView show];
//    [ToolMethod altermethord:[NSString stringWithFormat:@"最多可选择%zd张照片", selectMaxNum] andmessagestr:@"" andconcelstr:@"确定"];
}
