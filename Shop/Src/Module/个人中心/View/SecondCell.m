...
-(void)setSameModel:(DRSameModel *)sameModel
{
    ...
    self.countLabel.text =[NSString stringWithFormat:@"库存数(%@)：%.3f  发货地：%@",baseStr,sameModel.qty,sameModel.storeName] ;
}
...
-(void)setStandWithArray:(NSArray *)array
{
    ...
    for (int i = 0; i<array.count; i++) {
        CGSize tagTextSize = [array[i] sizeWithFont:ZF_FONT(12) maxSize:CGSizeMake(WScale(310),WScale(25))];
        if (tagBtnX+tagTextSize.width+WScale(25) >WScale(310)) {
