...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        _tableView.frame = CGRectMake(0, 0, ScreenW, ScreenH - (DRTopHeight+ 45));
...
- (void)setUpHeadView
{
    ...
    if (_saveType == DCSaveAdressChangeType) { //编辑
        ...
        _adressHeadView.provinceField.text =storeAddress[2];
        _adressHeadView.addressLabel.text = storeAddress[3];
        _adressHeadView.detailTextView.text = storeAddress[4];
    }
...
- (IBAction)saveNewAdressClick {
    ...
    if (_adressHeadView.rePersonField.text.length == 0 || _adressHeadView.rePhoneField.text.length == 0 || _adressHeadView.detailTextView.text.length == 0 || _adressHeadView.addressLabel.text.length == 0|| _adressHeadView.provinceField.text.length==0) {
    ...
    adressItem.provAddress = _adressHeadView.provinceField.text;
    adressItem.chooseAdress = _adressHeadView.selectBtn.titleLabel.text;
...
- (void)saveButtonBarItemClick
{
    ...
    if (_adressHeadView.rePersonField.text.length == 0 && _adressHeadView.rePhoneField.text.length == 0 &&_adressHeadView.provinceField.text.length==0&& _adressHeadView.selectBtn.titleLabel.text.length == 0 && _adressHeadView.detailTextView.text.length == 0) {
        [self.view makeToast:@"当前编辑为空" duration:0.5 position:CSToastPositionCenter];
        return;
    }
    NSString *adress = (_adressHeadView.selectBtn.titleLabel.text == nil) ? @"" : _adressHeadView.selectBtn.titleLabel.text; //取空
    NSArray *storeAddress = @[_adressHeadView.rePersonField.text,_adressHeadView.rePhoneField.text,_adressHeadView.provinceField.text,adress,_adressHeadView.detailTextView.text];
