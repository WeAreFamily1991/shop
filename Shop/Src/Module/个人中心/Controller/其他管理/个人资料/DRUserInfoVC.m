...
@interface DRUserInfoVC ()<UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0) {
        ...
        id imgStr1 = self.userModel.avatar?:@"1";
    ...
    {
        titleArray = @[@"用户名：",@"手机号：",@"公司名称：",@"公司地址：",@"公司电话：",@"联系人：",@"电话："];
        placeholdArray= @[@"请输入用户名",@"请输入手机号",@"请输入公司名称",@"请输入公司地址",@"请输入公司电话",@"请输入联系人",@"请输入电话"];
//        NSArray *contentArray = @[self.userModel.user_name,self.userModel.user_mobile,self.userModel.user_comany,self.userModel.user_address,self.userModel.company_phone,self.userModel.user_contacts,self.userModel.user_phone];
...
//    [self.view endEditing:YES];
    if (indexPath.row==1)
    {
        [self.navigationController pushViewController:[ChangeUserNameVC new] animated:YES];
