...
//#define maxOffsetY  (long)(150*kScreenWidth/375 + 48 - DRTopHeight)
@interface TakeawayShopView ()<ShopScrollViewDelegate,UIGestureRecognizerDelegate>
{
...
//                popupMenu.textColor = [UIColor grayColor];
...
//    [_shopImgView sd_setImageWithURL:[NSURL URLWithString:_shopModel.shopIcon] placeholderImage:IMAGENAMED(@"thumb") completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
- (void)setShopInfoView
{
    ...
    shopIcon.image = IMAGENAMED(@"icon_brd_big");
...
- (void)setActivityView
{
    ...
    if (activityList.count > 0) {
        ...
        if ([couponListModel.activeType integerValue] == 1) {
            iconView.image = IMAGENAMED(@"icon-discounts");
        }else if ([couponListModel.activeType integerValue] == 2)
        {
            iconView.image = IMAGENAMED(@"icon-sale");
        }
        else if ([couponListModel.activeType integerValue] == 3)
        {
            iconView.image = IMAGENAMED(@"icon-coupons");
        }
        ...
        nextImgView.image = IMAGENAMED(@"icon-folding");
        [activityView addSubview:nextImgView];
        //优惠活动数量
        UILabel *numLab = [UITool createLabelWithTextColor:[UIColor grayColor] textSize:10 alignment:NSTextAlignmentRight];
...
- (void)createDeatailInfoView
{
    ...
    if (couponList.count > 0) {
        ...
        for (int i = 0; i<couponList.count; i++) {
            ...
            couponImgView.image = IMAGENAMED(@"bg_coupon_red");
            if ([couponModel.couponNumber integerValue] <= 0)
            {
                couponImgView.image = IMAGENAMED(@"bg_coupon_yellow");
            }
            
            [couponView addSubview:couponImgView];
            //优惠券价格
            UILabel *priceLab = [UITool createLabelWithFrame:CGRectMake(14, 9, 100, 20) backgroundColor:[UIColor clearColor] textColor:RGBHex(0xFE5A2B) textSize:Size(20) alignment:NSTextAlignmentLeft lines:1];
            priceLab.text = [NSString stringWithFormat:@"¥%@",couponModel.couponPrice];
            [couponView addSubview:priceLab];
            //满多少钱可用
            UILabel *moreThanLab = [UITool createLabelWithFrame:CGRectMake(14, 40, 100, 14) backgroundColor:[UIColor clearColor] textColor:RGBHex(0xFD8F33) textSize:14 alignment:NSTextAlignmentLeft lines:1];
            moreThanLab.text = [NSString stringWithFormat:@"满%@可用",couponModel.couponCondition];
            [couponView addSubview:moreThanLab];
            //时间
            
            UILabel *timeLab = [UITool createLabelWithFrame:CGRectMake(14, 59, 140, 10) backgroundColor:[UIColor clearColor] textColor:RGBHex(0xFD8F33) textSize:10 alignment:NSTextAlignmentLeft lines:1];
    ...
    if (activityList.count > 0) {
        ...
        //
        UIImageView *nextImgView = [[UIImageView alloc]initWithFrame:CGRectMake(self.width-20, 13, 10, 10)];
        nextImgView.image = IMAGENAMED(@"icon_up_black");
    ...
    if (_shopModel.openHour.length == 0 || !_shopModel.openHour) {
        ...
        textLab.text = @"营业时间";
        //[UITool createLabelWithTextColor: textSize:12 alignment:NSTextAlignmentLeft]
        UILabel *timeLab =[UITool createLabelWithTextColor:[UIColor grayColor] textSize:12 alignment:NSTextAlignmentLeft];
    ...
    UILabel *contentLab = [UITool createLabelWithTextColor:[UIColor darkGrayColor] textSize:12 alignment:NSTextAlignmentLeft];
    ...
    contentLab.font =  DR_FONT(12);
    contentLab.numberOfLines = 0;
    contentLab.mj_w = self.width - (noticeLab.maxX+20);
    CGSize size = [AppMethods sizeWithFont: DR_FONT(12) Str:text withMaxWidth:contentLab.mj_w];
...
- (UIView *)setShopGradeAndCerViewWithType:(NSString *)type {
    ...
    CGSize gradeSize = [AppMethods sizeWithFont: DR_FONT(12) Str:grade withMaxWidth:100];
    gradeLab.frame = CGRectMake(max_X, 0, gradeSize.width+2, 18);
    [cerView addSubview:gradeLab];
    //超出预期
    UILabel *yuQiLab = [UITool createLabelWithTextColor:RGBHex(0xFFCD20) textSize:12 alignment:NSTextAlignmentLeft];
    yuQiLab.text = [ToolManager returnYuQiType:@"4.9"];
    CGSize yuQiSize = [AppMethods sizeWithFont: DR_FONT(12) Str:yuQiLab.text withMaxWidth:100];
    ...
    line.backgroundColor = RGBHex(0xF4F4F4);
    ...
    CGSize sellSize = [AppMethods sizeWithFont: DR_FONT(12) Str:sellCountLab.text withMaxWidth:100];
...
- (void)createMoveUpView
{
    ...
    imageView.image = IMAGENAMED(@"icon-Slidingback");
...
{
    if (!_productListView) {
        _productListView = [[ShopScrollView alloc]initWithFrame:CGRectMake(0,20, self.width, self.height-20) withShopModel:_shopModel withGroupID:_groupId currentVC:[self viewController]];
...
{
    UIView *navBarView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.width, DRTopHeight)];
    ...
    searchView.backgroundColor =[RGBHex(0xF4F4F4) colorWithAlphaComponent:0.0];
    ...
    _searchLab = [UITool createLabelWithFrame:CGRectMake(30, 2, 120, 20) backgroundColor:[UIColor clearColor] textColor:[UIColor grayColor] textSize:Size(12) alignment:NSTextAlignmentLeft lines:1];
    ...
    for (int i = 0; i<3; i++) {
        ...
        [itemBT setImage:IMAGENAMED(imgAray[i]) forState:UIControlStateNormal];
        ...
        if (i == 2) {
            if (_isCollect) {
                //已经收藏
                [itemBT setImage:IMAGENAMED(@"Already collected")   forState:UIControlStateNormal];
            }
        }
        if (i == 3) {
...
- (void)didScorll:(UIScrollView *)scorllView
{
    ...
    if (offsetY <= _maxOffset_Y && offsetY >= 0) {
        ...
        if (offsetY >= _startChange_Y) {
            ...
            if (imgRate2  >= 1) {
                [sousuBT setImage:IMAGENAMED(@"icon-search box") forState:UIControlStateNormal];
            }else{
                [sousuBT setImage:IMAGENAMED(@"sousuo_white") forState:UIControlStateNormal];
            }
            sousuBT.imageEdgeInsets = UIEdgeInsetsMake(8*imgRate2, 8*imgRate2, 8*imgRate2, 8*imgRate2);
            _searchView.mj_x = sousuBT.mj_x;
            _searchLab.mj_x = 30;
            _searchView.backgroundColor = [RGBHex(0xF4F4F4) colorWithAlphaComponent:alpha2];
            if (_alpha >= 0.8) {
                _searchLab.textColor = [[UIColor grayColor] colorWithAlphaComponent:_alpha];
            }else{
                _searchLab.textColor = [[UIColor grayColor] colorWithAlphaComponent:0];
            }
        ...
        _searchView.backgroundColor = [RGBHex(0xF4F4F4) colorWithAlphaComponent:0];
        _searchView.mj_x = sousuBT.mj_x;
        _searchView.mj_w =  37;
        _searchLab.mj_x = 30;
        _searchLab.textColor = [[UIColor grayColor] colorWithAlphaComponent:0];
    ...
    if (_alpha >= 0.5) {
        [_backBT   setImage:IMAGENAMED(@"back_grey")    forState:UIControlStateNormal];
        [messageBT setImage:IMAGENAMED(@"more_grey") forState:UIControlStateNormal];
        [collectBT setImage:IMAGENAMED(@"collect_grey") forState:UIControlStateNormal];
    }else{
        [_backBT   setImage:IMAGENAMED(@"icon_back_white") forState:UIControlStateNormal];
        [messageBT setImage:IMAGENAMED(@"more_white")   forState:UIControlStateNormal];
        [collectBT setImage:IMAGENAMED(@"collect_white")   forState:UIControlStateNormal];
    }
    if (_isCollect) {
        //已经收藏
        [collectBT setImage:IMAGENAMED(@"Already collected")   forState:UIControlStateNormal];
    }
    if (_alpha >= 0.8) {
...
- (void)navBarState
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        _searchView.mj_w =  37;
        [sousuBT   setImage:IMAGENAMED(@"sousuo_white") forState:UIControlStateNormal];
        [moreBT setImage:IMAGENAMED(@"more_white")   forState:UIControlStateNormal];
        [collectBT setImage:IMAGENAMED(@"collect_white")   forState:UIControlStateNormal];
        if (_isCollect) {
            //已经收藏
            [collectBT setImage:IMAGENAMED(@"Already collected")   forState:UIControlStateNormal];
...
{
    /*
     #define maxOffsetY (200 + 48 - DRTopHeight)
     #define maxOffsetY2 (200 + 48 - DRTopHeight - 28)
     */
    ...
    if (activityList.count > 0) {
        ...
        _maxOffset_Y = (Size(150) + 48 - DRTopHeight - 28);
        _startChange_Y = 21;
    }
...
//    [self addSubview:self.customheadView];
    //创建滚动视图
